...
export function ViewNFTs(props) {
    ...
    return (_jsxs(Container, { style: {
            minHeight: "300px",
        }, children: [_jsx(Container, { p: "lg", children: _jsx(ModalHeader, { title: connectLocale.viewFunds.viewNFTs, onBack: props.onBack }) }), _jsx(Line, {}), _jsxs(Container, { px: "sm", scrollY: true, style: {
                    maxHeight: "500px",
                }, children: [_jsx(Spacer, { y: "md" }), _jsx(Container, { style: {
                            display: "grid",
                            ...
                            return result.isLoading || !result.data ? (_jsx(Skeleton, { height: "160px", width: "160px" }, `${nftList[index]?.chain?.id}:${nftList[index]?.address}`)) : (result.data.map((nft) => (_jsx(NftCard, { ...nft, client: props.client, chain: nft.chain, theme: props.theme }, `${nft.chain.id}:${nft.address}:${nft.id}`))));
                        }) }), _jsx(Spacer, { y: "lg" })] })] }));
}
