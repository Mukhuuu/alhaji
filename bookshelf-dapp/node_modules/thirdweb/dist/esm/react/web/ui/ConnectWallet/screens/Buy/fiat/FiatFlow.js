...
export function FiatFlow(props) {
    ...
    if (screen.id === "onramp-status") {
        return (_jsx(OnrampStatusScreen, { title: props.title, client: props.client, intentId: props.quote.intentId, onBack: props.onBack, onViewPendingTx: props.onViewPendingTx, hasTwoSteps: hasTwoSteps, openedWindow: popupWindow, quote: props.quote, onDone: props.onDone, onShowSwapFlow: (_status) => {
                setScreen({ id: "postonramp-swap", data: _status });
            }, isBuyForTx: props.isBuyForTx, isEmbed: props.isEmbed }));
    }
    if (screen.id === "postonramp-swap") {
        return (_jsx(PostOnRampSwapFlow, { title: props.title, status: screen.data, quote: fiatQuoteToPartialQuote(props.quote), client: props.client, onBack: props.onBack, onViewPendingTx: props.onViewPendingTx, onDone: props.onDone, onSwapFlowStarted: () => {
                // no op
