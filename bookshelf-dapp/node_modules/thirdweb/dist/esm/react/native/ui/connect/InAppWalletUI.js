...
import { APPLE_ICON, DISCORD_ICON, EMAIL_ICON, FACEBOOK_ICON, GOOGLE_ICON, PHONE_ICON, } from "../icons/svgs.js";
...
export function InAppWalletUI(props) {
    ...
    const socialLogins = authOptions.filter((x) => x === "google" || x === "apple" || x === "facebook" || x === "discord");
    const [inputMode, setInputMode] = useState("email");
    return (_jsxs(View, { style: styles.container, children: [_jsx(View, { style: styles.row, children: socialLogins.map((auth) => (_jsx(SocialLogin, { auth: auth, ...props }, auth))) }), authOptions.includes("email") ? (inputMode === "email" ? (_jsx(PreOtpLogin, { auth: "email", ...props })) : (_jsx(ThemedButtonWithIcon, { theme: theme, title: "Email address", icon: EMAIL_ICON, onPress: () => setInputMode("email") }))) : null, authOptions.includes("phone") ? (inputMode === "phone" ? (_jsx(PreOtpLogin, { auth: "phone", ...props })) : (_jsx(ThemedButtonWithIcon, { theme: theme, title: "Phone number", icon: PHONE_ICON, onPress: () => setInputMode("phone") }))) : null] }));
}
